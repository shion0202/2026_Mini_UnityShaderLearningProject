...
///
/// 여기 없는 나머지 속성(60여 개)은 <see cref="Effect"/> / <see cref="Tweener"/> 를 통해
/// 직접 접근한다. 예) uiEffectController.Effect.gradationRotation = 90f;
///
/// 확장: 모든 public 조작 메서드는 virtual 이므로 상속으로 커스터마이즈할 수 있다.
/// 규모가 커지면 프리셋/트위너/속성을 별도 컴포넌트로 분리(Approach C)하는 것을 고려.
/// </summary>
...
public class UIEffectController : MonoBehaviour
{
    ...
    public class PresetEntry
    {
        ...
        /// <summary>id 가 비어 있으면 에셋 이름을 대체 키로 사용한다.</summary>
    ...
    /// <summary>제어 대상 UIEffectTweener(없을 수 있음). (고급 제어 직접 접근용)</summary>
    ...
    /// <summary>id 프리셋을 현재 이펙트 위에 강제로 겹쳐 적용한다(로컬→중앙 순서로 탐색, append 강제).</summary>
    ...
    /// <summary>프리셋 에셋을 직접 불러온다. (목록 등록 없이 참조로 바로 적용)</summary>
    ...
    /// <summary>이펙트를 기본값으로 초기화한다.</summary>
    ...
    /// <summary>역방향(1→0) 재생. resetTime=true 면 끝에서부터.</summary>
    ...
    /// <summary>현재 진행 방향의 반대로 재생(정방향↔역방향). 호버 인/아웃 같은 토글에 사용.</summary>
    ...
    /// <summary>트윈을 정지하고 시간을 리셋한다.</summary>
    ...
    /// <summary>트윈 재생 시간(초)을 바꾼다.</summary>
    ...
    /// <summary>반복 여부를 켜고 끈다(true=Loop, false=Once). PingPong 등 세부 모드는 Tweener.wrapMode 로 직접 설정.</summary>
    ...
    /// <summary>샘플링(블러/피셀레이트 등) 강도(0~1).</summary>
    ...
    /// <summary>트랜지션(디졸브/셔터 등) 진행도(0~1).</summary>
    ...
    /// <summary>그래디언트 오프셋(-1~1). 흐르는 그래디언트 연출에 사용.</summary>
    ...
    /// <summary>그래디언트 회전(0~360도).</summary>
    ...
    /// <summary>엣지 샤이니 진행도(0~1).</summary>
